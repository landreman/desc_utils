...
def test_iota_built_in_resolution():
    """
    Confirm that desc's built-in RotationalTransform objective function is
    approximately independent of grid resolution.
    """
    filenames = [
        ".//tests//inputs//LandremanPaul2022_QA_reactorScale_lowRes.h5",
    ...
    for filename in filenames:
        ...
        results = np.array(results)
        np.testing.assert_allclose(results, np.mean(results), rtol=1e-13)

def test_iota_built_in_value():
    """
    Confirm that desc's built-in RotationalTransform objective function 
